...
```lean
-- 类型推导示例
def example1 : Nat → Nat := λ x => x  -- 恒等函数

def example2 : Bool → Bool → Bool := λ x y => x  -- 投影函数

def example3 : (Nat → Bool) → Nat → Bool := λ f x => f x  -- 函数应用

-- 高阶函数示例
def twice {α : Type} (f : α → α) : α → α := λ x => f (f x)

def thrice {α : Type} (f : α → α) : α → α := λ x => f (f (f x))

-- 函数组合
def compose3 {α β γ δ : Type} (f : γ → δ) (g : β → γ) (h : α → β) : α → δ :=
  λ x => f (g (h x))
...
def uncurry {α β γ : Type} (f : α → β → γ) : α × β → γ :=
  λ p => f p.1 p.2
### 1.8.1.3.3 类型安全示例 / Type Safety Examples

```lean
-- 类型安全的函数定义
def safe_divide (n m : Nat) (h : m ≠ 0) : Nat := n / m

def safe_head {α : Type} (xs : List α) (h : xs ≠ []) : α :=
  match xs with
  | [] => absurd h (λ h' => h' rfl)
  | x :: _ => x

-- 类型安全的模式匹配
def safe_tail {α : Type} (xs : List α) : Option (List α) :=
  match xs with
  | [] => none
  | _ :: xs' => some xs'
...
## 1.8.1.5 理论意义与应用 / Theoretical Significance and Applications

### 1.8.1.5.1 数学基础 / Mathematical Foundations
...
### 1.8.1.5.2 计算机科学应用 / Computer Science Applications

- **编程语言**：类型系统和类型安全的理论基础
- **函数式编程**：纯函数和不可变数据的理论基础
- **编译器理论**：类型检查和类型推导的理论基础
- **程序验证**：程序正确性验证的理论基础

### 1.8.1.5.3 跨学科应用 / Interdisciplinary Applications
...
## 1.8.1.6 交叉引用 / Cross References

### 1.8.1.6.1 内部引用 / Internal References
...
### 1.8.1.6.2 外部资源 / External Resources
...
## 1.8.1.7 2025 规范对齐 / Alignment with Lean 4 (2025)

### 1.8.1.7.1 语法特性 / Syntax Features

- **STT实现**：STT 在 Lean 中通过 `inductive`/`def` 实现；函数类型 `A → B` 直接对应 λ-演算
- **类型推断**：类型推断基于 Hindley-Milner 算法；建议显式标注类型以避免歧义
- **函数式特性**：支持高阶函数、部分应用、柯里化等函数式编程特性

### 1.8.1.7.2 最佳实践 / Best Practices
...
### 1.8.1.7.3 扩展开发 / Extension Development
...
## 1.8.1.8 版本兼容性 / Version Compatibility

### 1.8.1.8.1 Lean版本支持 / Lean Version Support

- **Lean 3/4兼容**：基本语法在 Lean 3/4 间兼容；类型推断算法在 Lean 4 有所改进
- **类型类系统**：Lean 4 引入了更强大的类型类系统
- **元编程能力**：元编程能力在 Lean 4 中得到显著增强

### 1.8.1.8.2 兼容性考虑 / Compatibility Considerations
...
### 1.8.1.9.1 经典文献 / Classic Literature

- **Church论文**：简单类型理论的原始提出
- **λ-演算教材**：λ-演算的详细讨论
- **类型理论教材**：类型理论基础理论

### 1.8.1.9.2 现代资源 / Modern Resources

- **Lean 4 Reference Manual**：类型系统的实现示例
- **社区示例**：相关的社区实现
- **在线教程**：简单类型理论的学习资源

### 1.8.1.9.3 进一步阅读 / Further Reading
...
2. **《The Lambda Calculus: Its Syntax and Semantics》** - Henk Barendregt
   - λ-演算完整理论
   - 归约系统
   - 语义模型
