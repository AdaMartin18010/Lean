...
-- 程序即证明
def ProgramAsProof {α β : Type} (f : α → β) (a : α) : β := f a

-- 证明即程序
def ProofAsProgram {α β : Prop} (h : α → β) (ha : α) : β := h ha

-- 类型构造即逻辑构造
def TypeConstructionAsLogic {α β : Type} :
  (α → β) × (β → α) → (α ↔ β) :=
  λ ⟨f, g⟩ => ⟨f, g⟩
```
...
-- 否定引入规则
def negation_intro {α : Type} (f : α → Void) : α → Void := f

-- 否定消解规则
def negation_elim {α : Type} (a : α) (na : α → Void) : Void := na a
def implication_elim {α β : Type} (f : α → β) (a : α) : β := f a
...
## 1.11.3.4 理论意义与应用 / Theoretical Significance and Applications

### 1.11.3.4.1 数学基础 / Mathematical Foundations

Curry-Howard-Lambek对应为数学提供了统一的框架：

- **类型论**：类型系统和类型安全的理论基础
- **逻辑学**：直觉主义逻辑和构造性证明的理论基础
- **范畴论**：数学结构的抽象和统一处理
- **证明论**：形式化证明的理论基础

### 1.11.3.4.2 计算机科学应用 / Computer Science Applications

- **编程语言**：类型系统和程序语义的理论基础
- **定理证明**：自动定理证明和程序验证的理论基础
- **函数式编程**：纯函数和不可变数据的理论基础
- **并发理论**：并发程序和同步的理论基础

### 1.11.3.4.3 跨学科应用 / Interdisciplinary Applications
...
## 1.11.3.5 交叉引用 / Cross References
...
## 1.11.3.6 2025 规范对齐 / Alignment with Lean 4 (2025)

### 1.11.3.6.1 对应实现 / Correspondence Implementation

- **可验证映射**：将 CHL 对应落地为可验证映射：直积/直和/指数对象 ↔ `Prod`/`Sum`/`(→)` 之范畴解释
- **结构实例**：在 Lean 中以结构/实例呈现
- **模块隔离**：使用 `category` 相关实例（若依赖 mathlib4 的 `CategoryTheory`），需显式 `import` 并隔离到演示模块

### 1.11.3.6.2 最佳实践 / Best Practices

- **清晰对应**：类型、逻辑、范畴的对应关系应该清晰明确
- **类型安全**：保证类型安全性和一致性
- **可读性**：提高代码的可读性
- **模块化**：对应关系应该模块化设计

### 1.11.3.6.3 扩展开发 / Extension Development

- **统一框架**：支持类型、逻辑、范畴的统一框架
- **可扩展性**：支持对应关系的扩展
- **测试验证**：充分的测试和验证
- **文档说明**：详细的文档说明
...
## 1.11.3.7 版本兼容性 / Version Compatibility

### 1.11.3.7.1 Lean版本支持 / Lean Version Support

- **类型系统**：Curry-Howard-Lambek对应在Lean中自然体现
- **模块依赖**：若使用mathlib4的CategoryTheory模块，需标注版本兼容性
- **功能扩展**：Lean 4 提供了更多的功能扩展

### 1.11.3.7.2 兼容性考虑 / Compatibility Considerations

- **向后兼容**：保证向后兼容性
- **功能测试**：充分测试功能兼容性
- **文档更新**：及时更新相关文档

## 1.11.3.8 参考资料 / References

### 1.11.3.8.1 经典文献 / Classic Literature

- **Curry-Howard对应**：类型论与逻辑的对应关系
- **Lambek对应**：类型论与范畴论的对应关系
- **标准范畴论教材**：极限/指数对象章节的详细讨论

### 1.11.3.8.2 现代资源 / Modern Resources

- **Lean/Mathlib4 CategoryTheory文档**：范畴论的实现示例
- **社区示例**：相关的社区实现
- **在线教程**：Curry-Howard-Lambek对应的学习资源

### 1.11.3.8.3 进一步阅读 / Further Reading
