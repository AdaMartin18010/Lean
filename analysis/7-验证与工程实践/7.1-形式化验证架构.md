    ...
    S: Clone + Hash + Eq,
{
    ...
    pub fn explore_with_partial_order_reduction(
        ...
        // 伪实现：仅探索 ample 集合对应的后继，示意 POR 思想
        let mut queue = VecDeque::new();
        for &initial_id in &self.initial_states {
            queue.push_back(initial_id);
            self.visited.insert(initial_id);
        ...
        while let Some(current_id) = queue.pop_front() {
            let current_state = &self.states[&current_id];
            let actions = ample_set(current_state);
            // 根据动作筛选后继（此处简化为使用全部后继，真实实现需依据独立性/可交换性分析）
            if let Some(successors) = self.transitions.get(&current_id) {
                for &next_id in successors {
                    if !self.visited.contains(&next_id) {
                        self.visited.insert(next_id);
        ...
        ExplorationResult { states_explored: self.visited.len(), property_holds: counterexample.is_none(), counterexample }
    }
    ...
    where 
        A: Hash + Eq + Clone,
    {
        // 伪实现：在抽象域上进行去重搜索，示意抽象解释思想
        let mut queue = VecDeque::new();
        let mut seen_abstract: HashSet<A> = HashSet::new();
        for &initial_id in &self.initial_states {
            queue.push_back(initial_id);
            self.visited.insert(initial_id);
            let a0 = abstraction(&self.states[&initial_id]);
            seen_abstract.insert(a0);
        }
        let mut counterexample = None;
        while let Some(current_id) = queue.pop_front() {
            if let Some(successors) = self.transitions.get(&current_id) {
                for &next_id in successors {
                    let a = abstraction(&self.states[&next_id]);
                    if !seen_abstract.contains(&a) || refinement(&a, &self.states[&next_id]) {
                        seen_abstract.insert(a.clone());
                        if !self.visited.contains(&next_id) {
                            self.visited.insert(next_id);
                            queue.push_back(next_id);
                        }
